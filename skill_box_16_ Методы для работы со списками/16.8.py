...
print('Значит, выбывает каждый ' + str(K) + '-й человек')
N_list = list(range(1, 1 + N))
print('Текущий круг людей: ', N_list)
count = 0
while len(N_list) > 1:
    print('Текущий круг людей: ', N_list)
    print('Начало счёта с номера:', N_list[count])
    count = (count + K - 1) % len(N_list)
    if N_list[count] == N_list[-1]:
        print('Выбывает человек под номером:', N_list.pop(count))
        count = 0
    else:
        print('Выбывает человек под номером:', N_list.pop(count))
print('Остался человек под номером:', N_list[0])